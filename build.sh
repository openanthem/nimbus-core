...
function buildWithOutTests {
  ...
  mvn -D maven.repo.local=$HOME/.m2/repository clean install  -f platform.web/pom.xml -P devbuild -D skipDockerBuild -D maven.test.skip=true
}
...
function buildWithTests {
  ...
  mvn -D maven.repo.local=$HOME/.m2/repository clean install  -f platform.web/pom.xml -P devbuild -D skipDockerBuild
}
