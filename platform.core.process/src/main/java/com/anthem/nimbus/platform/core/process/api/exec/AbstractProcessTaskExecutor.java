...
//			}
	...
	protected <R> R crawlCommand(CommandMessage cmdMsg) {
		...
		if(cmd.getAction()==Action._new && !cmd.getRootDomainElement().hasRefId()) {
			PlatformSession.setAttribute(cmd, null);
		}
		
		Object cached = PlatformSession.getAttribute(cmd);
		...
		QuadModel<?,?> currentQModel = PlatformSession.getAttribute("/"+cmd.getRootDomainAlias());
		...
		PlatformSession.setAttribute(qLoadCmdMsg.getCommand(), q);
