...
public class ChildrenValidationAssignmentStrategy extends AbstractValidationAssignmentStrategy {
	...
	void assignGroupTo(Param<?> onChangeParam, Class<? extends ValidationGroup> targetGroup) {
		Model<?> nested = onChangeParam.findIfNested();
		if (null == nested) {
			throw new InvalidConfigException("Attempted to assign validation group to children params on a non-nested param.");
		}
		
		List<Param<?>> childrenParams = nested.getParams();
		if (null == childrenParams) {
			return;
		}
		
		for(Param<?> childParam: childrenParams) {
			this.handleNested(childParam, targetGroup, this::addGroupToParam);
		}
	}
	...
	void unassignGroupFrom(Param<?> onChangeParam, Class<? extends ValidationGroup> targetGroup) {
		Model<?> nested = onChangeParam.findIfNested();
		if (null == nested) {
			throw new InvalidConfigException("Attempted to unassign validation group to children params on a non-nested param.");
		}
		...
		for(Param<?> childParam: childrenParams) {
			this.handleNested(childParam, targetGroup, this::removeGroupFromParam);
		}
	}
