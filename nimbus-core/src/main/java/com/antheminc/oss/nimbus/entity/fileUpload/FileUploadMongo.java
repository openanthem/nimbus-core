...
public class FileUploadMongo implements FileUploadService {
	...
	public FileUploadMongo(BeanResolverStrategy beanResolver) {
		DefaultMongoModelRepository defaultMongoModelRepository = beanResolver.get(DefaultMongoModelRepository.class,
				"rep_mongodb");
	...
	public String upload(MultipartFile uploadFile) {
		...
		try {
			...
			throw new FrameworkRuntimeException(
					"Unable to upload File to GridFs with FileName " + uploadFile.getOriginalFilename(), e);
		}
	...
	public InputStream download(String downloadFileExternalId) {
		...
		if (null == gridFsFile) {
			...
			GridFsResource resource = new GridFsResource(gridFsFile,
					getGridFs().openDownloadStream(gridFsFile.getObjectId()));
			try {
				...
				throw new FrameworkRuntimeException(
						"Unable to write located file  with Id :" + downloadFileExternalId + " to InputStream " + e);
			}
		}
