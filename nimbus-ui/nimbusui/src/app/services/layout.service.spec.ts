...
describe('LayoutService', () => {
  ...
  it('getLayout() should call parseLayoutConfig()', async(() => {
    ...
    expect(service.parseLayoutConfig).toHaveBeenCalledWith(configServiceFlowConfigs.ownerlandingview.model);
  }));
  ...
  it('getLayout() should call parseLayoutConfig() even if flowName is not found', async(() => {
    ...
    expect(configService.setLayoutToAppConfigByModel).toHaveBeenCalledWith('test', 456);
  }));
  ...
  it('getLayout() should call parseLayoutConfig() even if flowName, flowRoodtId is not found', async(() => {
    ...
    expect(service.parseLayoutConfig).toHaveBeenCalledWith(456);
    expect(configService.setLayoutToAppConfigByModel).toHaveBeenCalledWith('test123', 456);
  }));
  ...
  it('getLayout() should call logger.error()', async(() => {
    ...
    service.getLayout('test');
    expect(logger.error).toHaveBeenCalledWith('ERROR: Unknown response for Layout config call - undefined');
  }));
  ...
  it('getFooterItems() should return object with links property', async(() => {
    layoutServicePageParam.type.model.params[0].type.model.params[0].config.uiNatures[0].attributes.value = 'LINK';
    const res = service.getFooterItems(layoutServicePageParam.type.model);
    expect(res.links).toBeTruthy();
  }));

  it('getFooterItems() should return object with sslCert property', async(() => {
    layoutServicePageParam.type.model.params[0].type.model.params[0].config.uiNatures[0].attributes.value = 'SSLCERT';
    const res = service.getFooterItems(layoutServicePageParam.type.model);
  ...
  it('getTopBar() should return object with headerMenus', async(() => {
    ...
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes = {'alias': 'Header'};
    layoutServicePageParam.type.model.params[0].config['initializeComponent'] = () => {return true;};
    const res = service.getTopBar(layoutServicePageParam.type.model);
    expect(res).toEqual({ branding: {}, headerMenus: [], accordions: [] });
    expect(service.parseTopBarConfig).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0].type.model, {}, [], []);
    expect(pageService.processEvent).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0].path, '$execute', new GenericDomain(), 'POST');
  }));
  ...
  it('parseTopBarConfig() should update branding{} with logo property', async(() => {
    ...
    layoutServicePageParam.type.model.params[0].config.uiNatures = [{attributes: { value: 'LOGO' }, name: 'ViewConfig.PageHeader'}]
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['logo']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with title property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'TITLE';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['title']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with appTitle property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'APPTITLE';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['appTitle']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with subTitle property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'SUBTITLE';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['subTitle']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with userName property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'USERNAME';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['userName']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with userRole property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'USERROLE';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['userRole']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with help property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'HELP';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['help']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with logOut property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'LOGOUT';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['logOut']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with linkNotifications property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'NOTIFICATIONS';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['linkNotifications']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} with numOfNotifications property', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'NUMBEROFNOTIFICATIONS';
    const branding = {};
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, [], []);
    expect(branding['numOfNotifications']).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('parseTopBarConfig() should update branding{} and headerMenus[]', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiNatures[0].attributes.value = 'MENU';
    const branding = {};
    const headerMenus = [];
    service.parseTopBarConfig(layoutServicePageParam.type.model, branding, headerMenus, []);
    expect(headerMenus[0]).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('getMenu() should return menuItems', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'MenuPanel';
  ...
  it('parseLayoutConfig() should call getTopBar(), getFooterItems(), getMenu(), getModalItems(), getActionTrayItems() and updaate layout$ subject', async(() => {
    ...
    expect(service.getTopBar).toHaveBeenCalledWith(flowModel.params[0].type.model);
    expect(service.getFooterItems).toHaveBeenCalledWith(flowModel.params[0].type.model);
    expect(service.getActionTrayItems).toHaveBeenCalledWith(flowModel.params[0].type.model);
    expect(service.getMenu).toHaveBeenCalledWith(flowModel.params[0].type.model);
    expect(service.getModalItems).toHaveBeenCalledWith(flowModel.params[0].type.model);
  ...
  it('buildMenu() should call createMenuItem(), createRouterLink() and update menuItems', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'MenuLink';
    spyOn(service, 'createMenuItem').and.returnValue({ 'routerLink': '', 'command': '', 'code': '' });
    ...
    expect(service.createMenuItem).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0]);
    expect(service.createRouterLink).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0]);
  }));
  ...
  it('buildMenu() should not call createRouterLink()', async(() => {
    ...
    expect(service.buildSubMenu).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0], createMenuItemResponse);
  ...
  it('buildSubMenu() should call createRouterLink()', async(() => {
    ...
    expect(service.createRouterLink).toHaveBeenCalledWith(layoutServicePageParam.type.model.params[0], true);
  }));
  ...
  it('createMenuItem() should return item', async(() => {
    ...
    expect(res).toEqual({"label":"testing label","path":"/home/vpHome","page":"testing page","icon":"testing imgSrc","imgType":"testing imgType","url":"testing url","type":"testing type","target":"testing target","rel":"testing rel","visible":true});
  }));

  it('createRouterLink() should return url from element', async(() => {
    layoutServicePageParam.config.uiStyles.attributes.url = '/test'
    expect(service.createRouterLink(layoutServicePageParam, true)).toEqual('/test');
  }));

  it('createRouterLink() should return updated url', async(() => {
    layoutServicePageParam.config.uiStyles.attributes.url = '/test'
    expect(service.createRouterLink(layoutServicePageParam, false)).toEqual('/h//test');
  }));
  ...
  it('processClick() should call pageService.processEvent()', async(() => {
    ...
    expect(pageService.processEvent).toHaveBeenCalledWith('testPath', '$execute', new GenericDomain(), 'GET');
  }));

  it('processClick() should not call pageService.processEvent()', async(() => {
    spyOn(URLUtils, 'getDomainPage').and.returnValue('test');
    spyOn(pageService, 'processEvent').and.returnValue('');
    service.processClick('', {'routerLink': '', 'page': 'test', 'path': 'testPath'});
    expect(pageService.processEvent).not.toHaveBeenCalled();
  }));

  it('getActionTrayItems() should return param', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'ActionTray';
    expect(service.getActionTrayItems(layoutServicePageParam.type.model)).toEqual(layoutServicePageParam.type.model.params[0]);
  }));

  it('getActionTrayItems() should return undefined', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'Header';
    expect(service.getActionTrayItems(layoutServicePageParam.type.model)).toBeFalsy();
  }));

  it('getModalItems() should return modalList with param', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'Modal';
    expect(service.getModalItems(layoutServicePageParam.type.model, [])).toEqual([layoutServicePageParam.type.model.params[0]]);
  }));

  it('getModalItems() should return empty modalList', async(() => {
    layoutServicePageParam.type.model.params[0].config.uiStyles.attributes.alias = 'Header';
    expect(service.getModalItems(layoutServicePageParam.type.model, [])).toEqual([]);
  }));
