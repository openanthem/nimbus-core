...
export class PageService {
  ...
  constructor(
    ...
    private toastService: NmMessageService,
  ...
  logError(err) {
    this.logger.error(
      'ERROR: Failure making server call : ' + JSON.stringify(err)
    );
  }
  ...
  loadDomainFlowConfig(flowName: string) {
    ...
    if (flowRoodtId != null) {
      ...
      baseUrl +=
        rootId == null ? '/' + flowName : '/' + flowName + ':' + rootId;
  ...
  loadFlowConfig(routeParms: any) {
    ...
    let url =
      baseUrl + '/' + Action._new.value + '?b=' + Behavior.execute.value;
    this.executeHttp(url, HttpMethod.GET.value, null);
  }
  ...
  getFlowNameFromPath(path: string): string {
    ...
    if (path.indexOf('/p/') > 0) {
      truncatedBasePath = path.substring(
        path.indexOf('/p/', 1) + 2,
        path.length
      );
    } else {
  ...
  processResponse(response: any) {
    ...
    while (response[index]) {
      ...
      if (subResponse.b === Behavior.execute.value) {
        let flowConfig = new Result(this.configService).deserialize(
          subResponse.result
        );
  ...
  findParamByCommandUri(inputCommandUri: string): Param {
    ...
    truncatedBasePath = inputCommandUri.substring(
      inputCommandUri.indexOf('/p/', 1) + 2,
      inputCommandUri.length
    );
    ...
    truncatedBasePath = truncatedBasePath.substring(
      0,
      truncatedBasePath.indexOf(action)
    );
    // remove the rootDomainid
    let domainFlowPath = '';
    if (truncatedBasePath.indexOf(':') > 0) {
      domainFlowPath = truncatedBasePath.substring(
        0,
        truncatedBasePath.indexOf(':')
      );
    } else {
      domainFlowPath = truncatedBasePath.substring(
        0,
        truncatedBasePath.indexOf(ServiceConstants.PATH_SEPARATOR, 1)
      );
    }

    // tslint:disable-next-line:max-line-length
    let pagePath = truncatedBasePath.substring(
      truncatedBasePath.indexOf(ServiceConstants.PATH_SEPARATOR, 1),
      truncatedBasePath.length
    );
  ...
  findParamByAbsolutePath(path: string): Param {
    ...
    if (pageParam) {
      ...
      relativeParamPath = nodes
        .slice(PAGE_NODE + 1)
        .join(ServiceConstants.PATH_SEPARATOR);
  ...
  getDetaultPageForFlow(model: Model): Param {
    if (model != null && model.params != null) {
      for (var p in model.params) {
        ...
        if (
          pageParam != null &&
          pageParam.config.uiStyles != null &&
          pageParam.config.uiStyles.name === ViewConfig.page.toString()
        ) {
      ...
      if (output.value == null || ParamUtils.isEmpty(output.value)) {
        ...
        if (output.action === Action._new.value) {
          ...
          if (
            ...
            this.logger.warn(
              'Received a _get call without model or config ' +
                output.value.path
            );
          }
    ...
    if (navToBrowserBack) {
      ...
      switch (navResponse.type) {
        ...
        default:
          ...
          let pageParam = this.findMatchingPageConfigById(
            navResponse.pageId,
            flow
          );
          this.navigateToPage(pageParam, flow);
  ...
  setViewRootAndNavigate(
    output: Result,
    flow: string,
    navToDefault: boolean,
    ...
    if (output.value.config.type.model.uiStyles) {
      viewRoot.layout =
        output.value.config.type.model.uiStyles.attributes.layout;
      // if(refreshLayout) {
  ...
  findMatchingPageConfigById(pageId: string, flowName: string): Param {
    ...
    if (!page) {
      this.logger.debug(
        'Page Configuration not found for Page ID: ' +
          pageId +
          ' in Flow: ' +
          flowName
      );
  ...
  processEvent(
    ...
    if (processUrl.indexOf('?') > 0) {
      url =
        url +
        processUrl.replace('{id}', this.entityId.toString()) +
        '&b=' +
        behavior;
    } else {
      url =
  ...
  executeHttp(url: string, method: string, model: any, paramPath?: string, showGridLoader?: boolean) {
    ...
    if (method !== '' && method.toUpperCase() === HttpMethod.GET.value) {
      ...
      method !== '' &&
      method.toUpperCase() === HttpMethod.POST.value
    ) {
  ...
  handleResponse(
    response: Observable<ExecuteResponse>,
    url: string,
    paramPath?: string,
    onSuccess?: () => void,
    onFailure?: () => void
  ...
  traverseFlowConfig(eventModel: ModelEvent, flowName: string) {
    ...
    if (viewRoot == undefined) {
      this.logger.warn(
        'Response cannot be processed for the path ' +
          eventModel.value.path +
          ' as there is no get/new done on the viewroot ' +
          flowName
      );
    } else {
      let flowConfig: Model = viewRoot.model;
  ...
  getUpdatedParamPath(eventModelPath: string): string {
    ...
    let updatedParamPath = eventModelPath.substring(
      eventModelPath.indexOf('/', 1) + 1
    );
    //Remove the context model attributes from path
    if (updatedParamPath.indexOf('/#') !== -1) {
      updatedParamPath = updatedParamPath.substring(
        0,
        updatedParamPath.indexOf('/#')
      );
    }
      ...
      // Loop through the Params in the Page
        ...
        rootParam.type.model.params.forEach(element => {
          ...
          if (
            element &&
            element.config &&
            element.config.code === paramTree[node]
          ) {
            const test = element.config.type && element.config.type.collection;
            if (
              (element.config.type && element.config.type.collection) ||
              node >= numNodes
            ) {
        ...
        this.logger.warn(
          'event update with path ' +
            eventModel.value.path +
            ' did not match with any param of current domain'
        );
      }
    }
  }
  ...
  processModelEvent(param: Param, eventModel: ModelEvent) {
    // Grid updates
    if (
      param.config.uiStyles != null &&
      ParamUtils.isTableBasedComponent(param)
    ) {
        ...
        if (param.path == eventModel.value.path) {
          ...
          if (eventModel.value.collectionElem) {
            if (param.tableBasedData.values == null) {
              ...
              param.tableBasedData.values.push(
                this.createTableBasedData(
                  eventModel.value.type.model.params,
                  param
                ).values
              );
          ...
          if (param.config.type.collection) {
            if (eventModel.value.page) {
              let page: GridPage = new GridPage().deserialize(
                eventModel.value.page
              );
              param.page = page;
            }
            param.tableBasedData = this.createTableBasedData(
          ...
          // Nested Collection. Need to traverse to right location
          ...
          if (
            ...
            eventModel.value.type &&
            eventModel.value.type.model
          ) {
            for (var p = 0; p < param.tableBasedData.values.length; p++) {
              if (param.tableBasedData.values[p]['elemId'] == elemIndex) {
                let nestedElement = this.getNestedElementParam(
                  param.tableBasedData.values[p]['nestedElement'],
                  nestedPath,
                  eventModel.value.path
                );
      ...
      param.config.uiStyles.attributes.alias ===
        ViewComponent.cardDetailsGrid.toString()
    ) {
      if (param.config.type.collection === true) {
        let payload: Param = new Param(this.configService).deserialize(
          eventModel.value,
          eventModel.value.path
        );
        if (payload.type.model && payload.path == param.path)
          // TODO - need to handle updates for each collection item in a card detail grid
  ...
  getNestedElementParam(
    ...
    if (this.matchNode(nestedElement, paramTree[startIndex])) {
      return this.traverseNestedPath(
        nestedElement,
        startIndex + 1,
        paramTree,
        eventPath
  ...
  traverseNestedPath(
    ...
    for (let p = 0; p < nestedElement.type.model.params.length; p++) {
      ...
      if (this.matchNode(element, tree[index])) {
        ...
        if (
          element.tableBasedData &&
          element.tableBasedData.values &&
          element.tableBasedData.values.length > 0
        ) {
          ...
          for (let i = 0; i < element.tableBasedData.values.length; i++) {
            if (
              this.matchElementId(
                element.tableBasedData.values[i],
                tree[index + 1]
              )
            ) {
              ...
              return this.traverseNestedPath(
                element.tableBasedData.values[i].nestedElement,
                index + 1,
                tree,
                eventPath
              );
  ...
  updateParam(sourceParam: Param, responseParam: Param) {
    ...
    if (config != null && config != undefined) {
      ...
      this.logger.debug(
        'Could not process the update from the server for ' +
          responseParam.path +
          ' because config is undefined.'
      );
    }
  }
  ...
  replaceSourceParamkeys(sourceParam: Param, responseParam: Param) {
    ...
    responseParamKeys.forEach(respKey => {
      try {
        ...
        if (
          respKey !== ParamAttribute.config.toString() &&
          respKey !== ParamAttribute.type.toString()
        )
          Reflect.set(
            sourceParam,
            respKey,
            Reflect.get(responseParam, respKey)
          );
      } catch (e) {
  ...
  updateNestedParameters(sourceParam: Param, responseParam: Param) {
    if (
      sourceParam.type &&
      sourceParam.type.model &&
      responseParam.type &&
      responseParam.type.model
    ) {
      try {
        if (sourceParam.type.model.params) {
          if (
            sourceParam.type.model.params.length <
            responseParam.type.model.params.length
          ) {
            this.logger.warn(
              'Detected new params for  ' +
                responseParam.path +
                ' in the update when sourceParam does not have config'
            );
          }
          for (var p in sourceParam.type.model.params) {
            this.updateParam(
              sourceParam.type.model.params[p],
              responseParam.type.model.params[p]
            );
          }
        }
      } catch (e) {
        this.logError(
          'Could not find source param to update the nested payload param path' +
            responseParam.path
        );
        this.logger.error(JSON.stringify(e));
      }
