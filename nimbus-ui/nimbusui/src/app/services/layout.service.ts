...
export class LayoutService {
  ...
  public getLayout(flowName: string) {
    ...
    if (layoutConfig) {
      ...
      return this.http.get(urlAction).subscribe(
        data => {
          let subResponse: any = data.result[0];
          if (subResponse) {
            let layoutConfig = new Result(this.configSvc).deserialize(
              subResponse.result
            );
            ...
            this.logger.error(
              'ERROR: Unknown response for Layout config call - ' +
                subResponse.b
            );
          }
        },
  ...
  public parseLayoutConfig(flowModel: Model) {
    ...
    const pageParam: Param = flowModel.params.find(
      p =>
        p.config &&
        p.config.uiStyles &&
        p.config.uiStyles.attributes &&
  ...
  private getFooterItems(layoutConfig: Model) {
    ...
    if (elemetarry.length != 0) footerConfig['links'] = elemetarry;
    return footerConfig;
  }
  ...
  private getTopBar(layoutConfig: Model) {
    ...
    layoutConfig.params.forEach(param => {
      ...
      if (
        attribute.alias === 'Header' ||
        attribute.alias === 'Global-Header' ||
        (attribute.alias === 'Section' && attribute.value === 'HEADER')
      ) {
        this.parseTopBarConfig(
          param.type.model,
          branding,
          headerMenus,
          accordions
  ...
  private parseTopBarConfig(
    ...
    topBarConfig.params.forEach(element => {
      if (
        element.config.type.nested === true &&
        element.config.uiNatures.length == 0
      ) {
        if (
  ...
  private getMenu(layoutConfig: Model): MenuPanel {
    ...
    for (let param of layoutConfig.params) {
      if (
        ...
        ViewComponent.menupanel.toString()
      ) {
  ...
  private buildMenu(param: Param, menuItems: MenuItem[]) {
    param.type.model.params.forEach(element => {
      ...
      if (
        ...
        ViewComponent.menulink.toString()
      ) {
  ...
  private buildSubMenu(param: Param, menuItem: MenuItem) {
    ...
    param.type.model.params.forEach(element => {
      ...
      if (
        ...
        item.command = (event: Event) => {
          this.processClick(event, item);
        };
  ...
  createRouterLink(element: Param, nested?: boolean): string {
    ...
    if (uiAttribute.url != null && uiAttribute.url != '') {
      if (nested) routeLink = uiAttribute.url;
      else routeLink = '/h/' + uiAttribute.url;
    }
  ...
  processClick(event: Event, item: MenuItem) {
    if (
      ...
      typeof item.routerLink === 'undefined'
    ) {
  ...
  private getActionTrayItems(layoutConfig: Model): Param {
    const actionTrayParam: Param = layoutConfig.params.find(
      param =>
        param.config.uiStyles != null &&
        param.config.uiStyles.attributes.alias ===
          ViewComponent.actiontray.toString()
    );
