...
export class ParamUtils {
  ...
  static isTableBasedComponent(param: Param): boolean {
    ...
    return TableComponentConstants.tableBasedComponents.includes(
   ...
   */
  public static applyLeafStateTransformations(
    obj: any,
    relativeParam: Param
  ): any {
    ...
    for (var x in obj) {
      ...
      if (x_param) {
        ...
        if (
          ...
          if (
            x_param.config.type &&
            ParamUtils.isKnownDateType(x_param.config.type.name)
          ) {
            transformed[x] = ParamUtils.convertServerDateStringToDate(
              obj[x],
              x_param.config.type.name
            );
          }
        }
      }
  ...
  public static findParamByPath(relativeParam: Param, key: string): Param {
    ...
    if (
      !(
        relativeParam.type &&
        relativeParam.type.model &&
        relativeParam.type.model.params
      )
    ) {
    ...
    if (key_parts.length === 1) {
      ...
      return ParamUtils.findParamByPath(
        params[0],
        key_parts.slice(1).join(ServiceConstants.PATH_SEPARATOR)
      );
    }
  }
  ...
  static getUiNature(param: Param, name: string): UiNature {
    ...
    return !uiNatures
      ? undefined
      : uiNatures.find(uiNature => uiNature.name === name);
  }
  ...
  static getUiNatures(param: Param): UiNature[] {
    if (
      param.config &&
      param.config.uiNatures &&
      param.config.uiNatures.length > 0
    ) {
   ...
   * @param pathToResolve
   */
  public static resolveParamUri(
    paramPath: string,
    pathToResolve: string
  ): string {
    if (
      pathToResolve === undefined ||
      pathToResolve === null ||
    ...
    if (pathToResolve.startsWith(`${ServiceConstants.PLATFORM_SEPARATOR}/`)) {
      console.error(
        'Cannot use cross domain uri here. Consider using @Config on a param for cross domain requests'
      );
      return null;
    }
    ...
    const currentTopNestedParamPath = this.resolvePathConstants(
      paramPath,
      currTopParamPathSegment
    );
    if (key_parts.length === 1) {
      return currentTopNestedParamPath;
    }
    return this.resolveParamUri(
      currentTopNestedParamPath,
      key_parts.slice(1).join(ServiceConstants.PATH_SEPARATOR)
    );
  }
  public static resolvePathConstants(
    currentPath: string,
    resolvePathSegment: string
  ): string {
    const currentPath_parts = currentPath.split(
    ...
    if (resolvePathSegment === ServiceConstants.SEPARATOR_URI_PARENT) {
      return currentPath_parts
        .slice(0, currentPath_parts.length - 1)
        .join(ServiceConstants.PATH_SEPARATOR);
    } else if (
      resolvePathSegment === ServiceConstants.MARKER_COMMAND_PARAM_CURRENT_SELF
    ) {
      // Ex: currentPath = /currentdomain/page/section/grid, resolvePathSegment = '<!#this!>' -> return '/currentdomain/page/section/grid'
      return currentPath;
    } else if (
      resolvePathSegment === ServiceConstants.SEPARATOR_URI_ROOT_DOMAIN
    ) {
      ...
      return currentPath
        .concat(ServiceConstants.PATH_SEPARATOR)
        .concat(resolvePathSegment);
    }
  }
    ...
    if (frmCtrl != null) {
      if (event.path === control.element.path) {
        ...
        if (
          ...
          control.requiredCss = ValidationUtils.rebindValidations(
            frmCtrl,
            event.activeValidationGroups,
            control.element
          );
        } else {
          control.requiredCss = ValidationUtils.applyelementStyle(
            control.element
          );
