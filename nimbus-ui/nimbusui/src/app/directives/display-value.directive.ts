...
export class DisplayValueDirective {
  ...
  ngOnInit() {
    if (this.config && this.config.uiStyles.attributes.applyValueStyles) {
      ...
      if (
        this.displayValue &&
        ((this.displayValue instanceof String &&
          this.displayValue.trim() !== '') ||
          this.displayValue === '')
      ) {
        this.renderer.addClass(
          this.el.nativeElement,
          this.getValue(this.displayValue)
        ); // Field Value
      } else {
        this.renderer.addClass(
          this.el.nativeElement,
          DisplayValueDirective.placeholder
        ); // placeholder Value
      }
    }
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (this.config && this.config.uiStyles.attributes.applyValueStyles) {
      ...
      if (changes.displayValue.previousValue === undefined) {
        ...
        this.renderer.removeClass(
          this.el.nativeElement,
          this.getValue(changes.displayValue.previousValue)
        );
      }
      // Add current value styles
      if (
        changes.displayValue.currentValue === undefined ||
        changes.displayValue.currentValue === '' ||
        (changes.displayValue.currentValue instanceof String &&
          changes.displayValue.currentValue.trim() === '') ||
        ...
        this.renderer.addClass(
          this.el.nativeElement,
          this.getValue(changes.displayValue.currentValue)
        );
      }
    }
  }
