...
export class PrintDirective {
  ...
  ngAfterViewInit() {
    this.subscription = this.printService.printClickUpdate$.subscribe(
      (event: PrintEvent) => {
        if (event.path === this.element.path) {
          if (event.printConfig.stylesheet || event.printConfig.useAppStyles) {
            event.printConfig.useDelay = true;
            event.printConfig.delay =
              event.printConfig.delay > PrintConfig.DEFAULT_DELAY
                ? event.printConfig.delay
                : PrintConfig.DEFAULT_DELAY;
            this.loggerService.debug(
              `Stylesheets found. useDelay will be set to true with delay of ${
                event.printConfig.delay
              }`
            );
          }
  ...
  private applyStylesToWindow(event: PrintEvent, printWindow: Window): void {
    ...
    if (event.printConfig.stylesheet) {
      var stylesheetURL =
        ServiceConstants.APP_BASE_URL + event.printConfig.stylesheet;
      ...
      this.loggerService.debug(
        `Added print stylesheet for: \"${stylesheetURL}\"`
      );
    }
  }
  ...
  private getPrintableContent(event: PrintEvent): string {
    ...
    if (this.contentSelector) {
      this.loggerService.debug(
        `Print feature is looking for parent via selector: \"${
      ...
      if (!printableElement) {
        throw new Error(
          `Unable to identify parent DOM element using \"${
            this.contentSelector
          }\"`
        );
      }
