...
export class LayoutResolver implements Resolve<ViewRoot> {
  ...
  resolve(
  ...
  ): Observable<ViewRoot> | Promise<ViewRoot> {
    ...
    if (flowConfig && flowConfig.model) {
      ...
      if (
        route.firstChild.params['pageId'] ||
        route.firstChild.params['subdomain']
      ) {
        routeToDefaultPage = false;
      }
      return this._pageSvc
        .getFlowLayoutConfig(flowName, routeToDefaultPage)
        .then(layout => {
          this._logger.debug(
            'layout resolver service flowName can be navigated' + flowName
          );
          return this._configSvc.getFlowConfig(flowName);
        });
    } else {
      this._pageSvc.getLayoutConfigForFlow(flowName);
      return this._pageSvc.layout$.pipe(
        map(layout => {
          this._logger.debug(
            'layout resolver service flowName can be navigated' + flowName
          );
