...
export class EventPropagationDirective implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.clicksubject.pipe(debounceTime(500)).subscribe(
      e => {
        this.clickEvnt.emit(e);
      },
      err => {
        this.logger.error('Failed to emit click event' + JSON.stringify(err));
      }
    );
    ...
    this.postProcessing = this.pageService.postResponseProcessing$
      ...
      .subscribe(
        ...
        err => {
          this.logger.error(
            'Failed on processing eventpropagation for path ' +
              event +
              ' with ' +
              JSON.stringify(err)
          );
