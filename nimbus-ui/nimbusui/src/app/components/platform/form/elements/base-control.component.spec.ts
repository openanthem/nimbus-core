...
const declarations = [BaseControlClass];
const imports = [HttpModule, HttpClientTestingModule];
const providers = [
...
describe('BaseControl', () => {
  ...
  beforeEach(() => {
    ...
    fg.addControl(
      fieldValueParam.config.code,
      new FormControl(fieldValueParam.leafState, checks)
    );
  ...
  it('ngOnInit() should not update requiredCss Property from ValidationUtils.rebindValidations()', () => {
    fixture.whenStable().then(() => {
      ...
      hostComponent.element.config.validation.constraints[0].attribute.groups = [
        '1'
      ];
  ...
  it('ngAfterViewInit() should not call controlSubscribers.validationUpdateSubscriber() and call controlSubscribers.onChangeEventSubscriber()', async(() => {
    ...
    hostComponent.ngAfterViewInit();
    expect(
      controlSubscribers.validationUpdateSubscriber
