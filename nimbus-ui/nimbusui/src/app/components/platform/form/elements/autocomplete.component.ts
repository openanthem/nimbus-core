...
@Component({
  ...
  <ng-template [ngIf]="element?.visible == true">
        ...
        <p-autoComplete [disabled]="!element?.enabled" [(ngModel)]="value"
...
export class NmAutocomplete extends BaseControl<any> {
    ...
    ngOnInit() {
        console.log("oninit called from tests");
        super.ngOnInit();
    ...
    sync(value) {
        let obj = this.suggestionObj.find(obj => (obj[this.element.config.uiStyles.attributes.display] == value));
        let postObject: any;
        if (this.form === undefined) {
            postObject = obj;
        }
        else if (this.form && (this.form.controls[this.element.config.code] != null && this.form.controls[this.element.config.code].valid)) {
        ...
        if (this.element.config.uiStyles.attributes.postEventOnChange) {
            this.pageSvc.postOnChange(this.element.path, 'state', JSON.stringify(postObject));
        }
    }
