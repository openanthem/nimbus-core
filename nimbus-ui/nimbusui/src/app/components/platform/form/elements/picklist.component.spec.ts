...
const declarations = [OrderablePickList, InputLabel, TooltipComponent];
const imports = [
  PickListModule,
  HttpModule,
...
describe('OrderablePickList', () => {
  ...
  beforeEach(() => {
    ...
    fg.addControl(
      pickListElement.config.code,
      new FormControl(pickListElement.leafState, checks)
    );
  ...
  it('emitValueChangedEvent() should update the controlValueChanged eventemitter', () => {
    fixture.whenStable().then(() => {
      ...
      hostComponent.form.controls[
        this.hostElement.element.config.code
      ].setValue('s');
