...
@Component({
  ...
  template: `
    ...
    <p-editor
      ...
      [readonly]="
        !element?.enabled || element?.config?.uiStyles?.attributes?.readOnly
      "
      [attr.readonly]="
        element?.config?.uiStyles?.attributes?.readOnly ? true : null
      "
    ...
    >
      <p-header [hidden]="element?.config?.uiStyles?.attributes?.readOnly">
        <ng-template
          ngFor
          let-toolbarFeature
          [ngForOf]="element?.config?.uiStyles?.attributes?.toolbarFeatures"
        >
...
export class RichText extends BaseControl<String> {
  ...
  ngOnInit() {
    ...
    let headingConfig = this.buildConfigFromNature(
      ViewConfig.headings.toString()
    );
    if (headingConfig) {
