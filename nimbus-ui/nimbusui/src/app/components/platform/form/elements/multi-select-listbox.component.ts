...
@Component({
  ...
  template: `
    <div [formGroup]="form" [hidden]="!element?.visible">
      ...
      <p-listbox
        [options]="optionsList"
        formControlName="{{ element.config.code }}"
        multiple="multiple"
        (onChange)="emitValueChangedEvent(this,value)"
        checkbox="checkbox"
        filter="filter"
        [style]="{ width: '190px', 'max-height': '250px' }"
      ></p-listbox>
    </div>
  `
})
      ...
      this.subscribers.push(this.form.controls[this.element.config.code].valueChanges.subscribe(
        $event => {
          ...
          if (
            ...
            this.form.controls[this.element.config.code].invalid &&
            !this.form.controls[this.element.config.code].pristine
          ) {
            this.counterMessageService.evalFormParamMessages(this.element);
            this.sendEvent = true;
            this.counterMessageService.evalCounterMessage(true);
    ...
    this.controlValueChanged.subscribe($event => {
      if ($event.config.uiStyles.attributes.postEventOnChange) {
        ...
        this.pageService.processEvent(
          this.element.config.uiStyles.attributes.postButtonUrl,
          null,
          $event.leafState,
