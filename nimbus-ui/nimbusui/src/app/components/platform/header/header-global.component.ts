...
export class HeaderGlobal {
  ...
  toggleOpen(e: any) {
    ...
    if (selectedDropDownState == 'openPanel') {
      ...
      if (
        this.dropDowns &&
        (this.mouseEventSubscription == undefined ||
          this.mouseEventSubscription.closed)
      )
        this.mouseEventSubscription = observableFromEvent(document, 'click')
          .pipe(
            filter(
              (event: any) =>
                !this.isClickedOnDropDown(
  ...
  get headerImageURL() {
    if (
      !this.branding ||
      !this.branding.logo.config ||
      !this.branding.logo.config.uiStyles
    ) {
      return undefined;
    }
    ...
    return (
      this.imagesPath + this.branding.logo.config.uiStyles.attributes.imgSrc
    );
  }
}
