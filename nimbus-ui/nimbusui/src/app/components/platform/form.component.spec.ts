...
describe('Form', () => {
  ...
  it('nm-counter-message should be created if the showmessages attribute is configured as true', async(() => {
    ...
    const nmCounterMessageEle = debugElement.query(
      By.css('nm-counter-message')
    );
    expect(nmCounterMessageEle.attributes['ng-reflect-form']).toBeTruthy();
    expect(
      nmCounterMessageEle.attributes['ng-reflect-form'].length
    ).not.toEqual(0);
  ...
  it('On updating the textbox value the form should be valid', async(() => {
    const newElement = Object.assign({}, formElement);
    const newModel = Object.assign({}, formModel);
    newElement.type.model.params[2].config.type.model.paramConfigIds = [
      '537',
      '538',
      '539',
      '540',
    ...
    newElement.type.model.params[2].type.model.params.push(
      textboxnotnullelement
    );
    newModel.params[2].config.type.model.paramConfigIds = [
      '537',
      '538',
      '539',
      '540',
    ...
    const textBox = fixture.debugElement.query(
      By.css('.form-control.text-input')
    ).nativeElement;
    textBox.value = 'test-436';
    textBox.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    expect(hostComponent.form.valid).toBeTruthy();
    expect(hostComponent.form.value).toEqual({
      medications1: '',
      question1notnull: 'test-436'
    });
  }));
  ...
  it('ngOnInit() should update the elementCss property if element.config.uiStyles.attributes.cssClass is inline', () => {
    fixture.whenStable().then(() => {
      ...
      hostComponent.ngOnInit();
      expect(hostComponent.elementCss).toEqual(
        'd-block d-md-inline-block mr-3'
      );
    });
  });
  ...
  it('buildFormElements() should subscribe to pageService.eventUpdate$', async(() => {
    ...
    const eve = {
      leafState: { a: 'a' },
      path: 'test',
      config: { uiStyles: { attributes: { alias: 'Form' } } }
    };
