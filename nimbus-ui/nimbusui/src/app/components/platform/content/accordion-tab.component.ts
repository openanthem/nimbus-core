...
@Component({
  ...
  template: `
    <p-accordionTab
      [selected]="element?.config?.uiStyles?.attributes?.selected"
    ...
    >
      ...
      <div
        class="accordionBtn"
        *ngIf="element?.config?.uiStyles?.attributes?.editable"
      >
      ...
      <ng-template [ngIf]="form === undefined">
        ...
        >
          <!-- ButtonGroup -->
          <ng-template
            [ngIf]="tabElement.alias == componentTypes.buttonGroup.toString()"
          >
            <div class="">
              <nm-button-group
          ...
          >
            <nm-link id="{{ tabElement.config?.code }}" [element]="tabElement">
            </nm-link>
          </ng-template>
          <!-- Grid Param -->
          <ng-template
          ...
          <!-- Card Content -->
          <ng-template
            [ngIf]="tabElement.alias == componentTypes.cardDetail.toString()"
          >
            <nm-card-details
              id="{{ tabElement.config?.code }}"
              [element]="tabElement"
...
export class AccordionTab extends BaseElement {
  ...
  getInfoText() {
    for (let i = 0; i < this.element.type.model.params.length; i++) {
      if (
        this.element.type.model.params[i].alias ===
        ViewComponent.tabInfo.toString()
      ) {
        ...
        return this.element.type.model.params[i].config.uiStyles.attributes
          .info;
      }
    }
  }
