    ...
    <ng-template [ngIf]="!imgSrc && element?.visible == true">
      ...
      <ng-template [ngIf]="value == componentTypes.menu.toString()">
          ...
          >
            <svg class="">
              <use
                ...
                attr.xlink:href="{{ imagesPath }}{{ imgSrc }}#Layer_1"
              ></use>
            </svg>
          </a>
        </ng-template>
        ...
        <div class="dropdown-menu dropdown-menu-right">
          ...
          >
            <ng-template
              [ngIf]="
                menuParam.config?.uiStyles?.attributes?.alias ==
                componentTypes.menu.toString()
              "
            >
              <ng-template
                ngFor
                let-linkParam
                [ngForOf]="menuParam?.type?.model?.params"
              >
                <nm-link
                  [element]="linkParam"
                  inClass="dropdown-item"
                ></nm-link>
              </ng-template>
            </ng-template>
          </ng-template>
      ...
      <ng-template [ngIf]="value == componentTypes.default.toString()">
        <a
          class="{{ cssClass }}"
          href="javascript:void(0)"
          (click)="processOnClick(element.path, method, b)"
        >
          {{ this.label }}
        </a>
      </ng-template>
      <ng-template [ngIf]="value == componentTypes.inline.toString()">
        <a
          class="{{ cssClass }}"
          href="javascript:void(0)"
          title="{{ this.element.leafState }}"
          (click)="processOnClick(element.path, 'GET', '$executeAnd$nav')"
        >
  ...
  ngOnInit() {
    ...
    if (
      ...
      if (this.rowData) {
        ...
        let urlParams: string[] = this.getAllURLParams(
          this.element.config.uiStyles.attributes.url
        );
  ...
  resolveUrl() {
    ...
    if (this.element.leafState) {
      return resolvedUrl.replace(
        new RegExp('{' + this.element.config.code + '}.*', 'g'),
        this.element.leafState
      );
    } else {
      return resolvedUrl;
  ...
  processOnClick(uri: string, httpMethod: string, behaviour: string) {
    ...
    if (this.root) {
      this.root.type.model.params.forEach(ele => {
        if (
          ele.path ===
          this.element.path.slice(0, this.element.path.lastIndexOf('/')) + '/id'
        ) {
