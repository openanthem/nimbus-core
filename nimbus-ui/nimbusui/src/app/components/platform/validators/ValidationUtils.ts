...
export class ValidationUtils {
  static buildStaticValidations(element: Param): ValidatorFn[] {
    ...
    if (element.config.validation) {
      element.config.validation.constraints.forEach(constraint => {
        if (
          (constraint.attribute != null &&
            constraint.attribute.groups == null) ||
          constraint.attribute.groups.length == 0
        ) {
  ...
  static buildDynamicValidations(
    element: Param,
    groups: String[]
  ): ValidatorFn[] {
    var checks: ValidatorFn[] = [];
  ...
  static createRequired(element: Param, groups: String[]): boolean {
    ...
    if (element.config.validation) {
      element.config.validation.constraints.forEach(validator => {
        groups.forEach(group => {
          if (
            validator.attribute.groups.some(x => x === group) &&
            validator.name == ValidationConstraint._notNull.value
          ) {
            required = true;
          }
        });
  ...
  static applyelementStyle(element: Param): boolean {
    ...
    if (element && element.config && element.config.validation) {
      element.config.validation.constraints.forEach(validator => {
        if (
          (validator.name === ValidationConstraint._notNull.value ||
            validator.name === ValidationConstraint._notEmpty.value) &&
          validator.attribute != null &&
          validator.attribute.groups.length == 0
        ) {
          //style = 'required';
          requiredCss = true;
        }
