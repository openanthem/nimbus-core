...
describe('DataTable', () => {
  ...
  it('Filtering should call the inputfilter method', async(() => {
    ...
    expect(hostComponent.inputFilter).toHaveBeenCalledWith(
      event,
      hostComponent.dt,
      'petName',
      'equals'
  ...
  it('Onclick of the clear button the clearFilter method should be called and clear the input', async(() => {
    ...
    expect(hostComponent.clearFilter).toHaveBeenCalledWith(
      allInputEle[2],
      hostComponent.dt,
      'ownerName'
    );
  ...
  it('header Div is created if showHeader attribute is configured as true  ', async(() => {
    ...
    const headerDivEle = debugElement.query(
      By.css('.ui-datatable-header.ui-widget-header')
    );
  ...
  it('Clear Grid Filters button should be created if clearAllFilters configured as true', async(() => {
    ...
    expect(allBtnEles[0].nativeElement.innerText.toString()).toEqual(
      'Clear Grid Filters'
    );
  }));
  ...
  it('Clear Grid Filters button should not be created if clearAllFilters configured as false', async(() => {
    ...
    expect(allBtnEles[0].nativeElement.innerText.toString()).not.toEqual(
      'Clear Grid Filters'
    );
  }));
  ...
  it('filter icon should be created if the filter attribute is configured as true', async(() => {
    fixture.detectChanges();
    const debugElement = fixture.debugElement;
    const filterIconEle = debugElement.queryAll(
      By.css('.fa.fa-fw.fa-filter.hidden-md-down')
    );
  ...
  it('row expander icon should be created if expandableRows attribute is configured as true', async(() => {
    fixture.detectChanges();
    const debugElement = fixture.debugElement;
    const expenderIconEle = debugElement.query(
      By.css('.fa.fa-fw.fa-chevron-circle-right.ui-row-toggler')
    );
  ...
  it('nm-action-dropdown should not be created if linkMenu is not configured', async(() => {
    fixture.detectChanges();
    let debugElement = fixture.debugElement;
    const previousActionDropdownEle = debugElement.queryAll(
      By.css('nm-action-dropdown')
    );
    hostComponent.params[8].uiStyles.attributes.alias = '';
    fixture.detectChanges();
    debugElement = fixture.debugElement;
    const actionDropdownEle = debugElement.queryAll(
      By.css('nm-action-dropdown')
    );
    expect(
      previousActionDropdownEle.length > actionDropdownEle.length
    ).toBeTruthy();
  }));
  ...
  it('On updating the records the pagination should be updated', async(() => {
    ...
    pageService.emitGridValueUpdate(tableGridValueUpdate);
    fixture.detectChanges();
    const paginatorAnchorTags = debugElement.queryAll(
      By.css(
        'a.ui-paginator-page.ui-paginator-element.ui-state-default.ui-corner-all'
      )
    );
    ...
    tableGridValueUpdate.tableBasedData.values.push(
      tableGridValueUpdate.tableBasedData.values[0]
    );
    pageService.emitGridValueUpdate(tableGridValueUpdate);
    fixture.detectChanges();
    const updatedPaginatorAnchorTags = debugElement.queryAll(
      By.css(
        'a.ui-paginator-page.ui-paginator-element.ui-state-default.ui-corner-all'
      )
    );
  ...
  it('On click of the nm-header-checkbox should add the ui-state-active class to all the checkboxes and update selectedRows', async(() => {
    ...
    fixture.detectChanges();
    let debugElement = fixture.debugElement;
    const allCheckBoxEles = debugElement.queryAll(
      By.css('.ui-chkbox-box.ui-widget')
    );
    allCheckBoxEles[0].nativeElement.click();
    fixture.detectChanges();
    for (let i = 0; i < allCheckBoxEles.length; i++) {
      expect(
        allCheckBoxEles[i].nativeElement.classList[3] == 'ui-state-active'
      ).toBeTruthy();
      expect(
        allCheckBoxEles[i].nativeElement.classList.length == 4
      ).toBeTruthy();
    }
    expect(hostComponent.selectedRows).toEqual(
      hostComponent.element.tableBasedData.values
    );
  }));
  ...
  it('td with no records is found string should not be created if data is available', async(() => {
    ...
    for (let i = 0; i < tdAll.length; i++) {
      expect(tdAll[i].nativeElement.innerText.toString()).not.toEqual(
        'No records found'
      );
    }
  }));
  ...
  it('post button should be created is postButton attribute is configured as true', async(() => {
    ...
    fixture.detectChanges();
    const debugElement = fixture.debugElement;
    const postBtnEle = debugElement.query(
  ...
  it('Onclick of post button should call postGridData() and pageService.processEvent', async(() => {
    ...
    const resolvedPostButtonUri = ParamUtils.resolveParamUri(
      ...
      hostComponent.element.config.uiStyles.attributes.postButtonUri
    );
    const item: GenericDomain = new GenericDomain();
    item.addAttribute(
      hostComponent.element.config.uiStyles.attributes.postButtonTargetPath,
      ['0', '1']
    );
    const postBtnEle = debugElement.query(
      By.css('.btn.btn-secondary.post-btn')
  ...
  it('On click of the filter icon the tr with filters should be created', async(() => {
    fixture.detectChanges();
    const debugElement = fixture.debugElement;
    const filterIconEle = debugElement.query(
      By.css('.fa.fa-fw.fa-filter.hidden-md-down')
    );
  ...
  it('ngOnInit() should update the hasFilters, and rowExpanderKey properties', () => {
    ...
    const eleConfig = {
      code: '',
      uiStyles: { attributes: { rowSelection: true } }
    };
  ...
  it('ngAfterViewInit() should call pageService.processEvent()', () => {
    fixture.whenStable().then(() => {
      const eleConfig = {
        code: '',
        uiStyles: { attributes: { onLoad: true } }
      };
  ...
  it('ngAfterViewInit() should update the value, totalRecords properties and call updatePageDetailsState()', () => {
    fixture.whenStable().then(() => {
      const eleConfig = {
        code: '',
        uiStyles: { attributes: { onLoad: true, lazyLoad: true } }
      };
      hostComponent.element.path = 'test';
      const eve = {
        tableBasedData: { values: [] },
        path: 'test',
        gridList: [],
        page: { totalElements: 100, first: true }
      };
      spyOn(configService, 'getViewConfigById').and.returnValue(eleConfig);
  ...
  it('ngAfterViewInit() should not call updatePageDetailsState()', () => {
    ...
    const eve = {
      path: '1test',
      gridList: 'tGrid',
      page: { totalElements: 'telements', first: true }
    };
    spyOn(configService, 'getViewConfigById').and.returnValue(eleConfig);
    spyOn(hostComponent, 'updatePageDetailsState').and.callThrough();
    hostComponent.ngAfterViewInit();
  ...
  it('ngAfterViewInit() should call updatePageDetailsState() and update the dt.first', () => {
    const eleConfig = {
      code: '',
      uiStyles: { attributes: { onLoad: true, lazyLoad: false } }
    };
    hostComponent.element.path = 'test';
    const eve = {
      tableBasedData: { values: '' },
  ...
  it('ngAfterViewInit() should not call updatePageDetailsState() based on eve.page.first', () => {
    ...
    const eve = {
      ...
      path: 'test',
      gridList: 'tGrid',
      page: { totalElements: 'telements', first: false }
    };
    spyOn(configService, 'getViewConfigById').and.returnValue(eleConfig);
    spyOn(hostComponent, 'updatePageDetailsState').and.callThrough();
    hostComponent.ngAfterViewInit();
  ...
  it('ngAfterViewInit() should call form.controls.t.enable()', () => {
    fixture.whenStable().then(() => {
      const eleConfig = {
        code: 't',
        uiStyles: { attributes: { onLoad: false } }
      };
      hostComponent.element.path = '/test';
      const eve = {
        enabled: true,
  ...
  it('ngAfterViewInit() should not call form.controls.t.disable()', () => {
    fixture.whenStable().then(() => {
      ...
      pageService.notifyErrorEvent(eve);
      expect(
        hostComponent.form.controls.firstName.disable
  ...
  it('isClickedOnDropDown() should return true', () => {
    ...
    dArray['elementRef'].nativeElement.contains = () => {
      return true;
    };
  ...
  it('postGridData() should call the pageService.processEvent()', () => {
    fixture.whenStable().then(() => {
      const eleConfig = {
        code: '',
        uiStyles: { attributes: { postButtonTargetPath: true } }
      };
      spyOn(configService, 'getViewConfigById').and.returnValue(eleConfig);
  ...
  it('customSort() should not sort value array for number type', () => {
    const eve = {
      ...
      field: {
        code: 'test',
        uiStyles: { attributes: { sortAs: 'NUMBER' } },
        type: { name: 't' }
      }
    };
  ...
  it('customSort() should not sort value array for Date type', () => {
    const eve = {
      ...
      field: {
        code: 'test',
        uiStyles: { attributes: { sortAs: 'DATE' } },
        type: { name: 't' }
      }
    };
  ...
  it('toggleOpen() should not call the mouseEventSubscription.unsubscribe()', () => {
    ...
    hostComponent.toggleOpen(eve);
    expect(
      hostComponent.mouseEventSubscription.unsubscribe
    ).not.toHaveBeenCalled();
  });
  ...
  it('export() should call dt.exportCSV()', () => {
    ...
    (hostComponent as any).dt = {
      filteredValue: [{ a: tDate }],
      value: [],
      exportCSV: () => {}
    };
    hostComponent.params = [new ParamConfig(configService)];
  ...
  it('td with no records is found string should be created if data is not available', async(() => {
    ...
    expect(tdAll.nativeElement.innerText.toString()).toEqual(
      'No records found'
    );
  }));
});
